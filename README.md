...
* [安装脚本部分](#安装脚本部分)
   ...
   * [软件批量安装](#软件批量安装)
   * [Vim及插件安装](#vim及插件安装)
   * [Vim-YouCompleteMe插件编译安装](#youcompleteme编译安装)
* [工具脚本部分](#工具脚本部分)
...
- **安装ArchLinux或软件**：在repo根目录，运行[install.sh](./install.sh)，选择所需的功能，根据提示进行操作。详细内容见[下文介绍](#安装脚本部分)。
    ...
    ./install.sh  
    ```
    :exclamation: 注意：除了下文单独列举的（如Arch安装第一步）一些功能之外，其他脚本都建议从install.sh统一执行，否则可能出错
- **使用我的[Vim配置](./home_k/.vimrc)**：Unix环境直接运行以下命令，等待自动安装完成即可
    ```bash
...
Vim比较特殊，尤其是YCM经常安装失败，所以单独列出来，安装见[Usage](#usage)
