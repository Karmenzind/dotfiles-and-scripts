...
- Expose Unix GVM through a lazy `gvm` wrapper that loads the real implementation only when invoked from a directory directly containing `go.mod`; do not source GVM globally (including via `~/.bashrc`) or from directory-change hooks. After a deliberate load, remove GVM's `cd` override so Claude Code snapshots cannot retain `cd` without `_encode`/`_decode`. See `docs/unix-project-environments.md`.
